...
function cliParser(argv) {
  ...
  parsed['local-user'] = parsed['local-user'] || process.env.USER;
  parsed['local-host'] = parsed['local-host'] || defaultHost();
...
function defaultHost() {
  var isIP4 = { family: 'IPv4' };
  return _.where(interfaces(), isIP4)[0].address;
}

function interfaces() {
  var ifaces = os.networkInterfaces();
  // eslint-disable-next-line no-irregular-whitespace
  return ifaces.eth0 || ifaces.en0 || _.first(ifaces);
}
